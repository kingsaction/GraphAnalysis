...
图分析系统又名关联分析系统，该系统的研发旨在支持大规模图数据的交互式可视化分析，图数据在当今世界数量及其庞大，怎么利用这些数据成为数据科学家不得不考虑的问题，
...
该仓库的代码版本不再发生变化，新的优化工作和代码将会创建一个新的仓库，并将该代码全部clone下来，在新的仓库里面进行新版本的迭代。
...
* 未实现的功能
    * 目前仅仅支持3000个点、3000条边左右的数据量，从当前数据量的情况，这远远没达到大规模图数据分析的要求，下一步采用增量技术实现对大规模数据集的支持；
