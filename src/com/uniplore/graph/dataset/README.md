...
```
-- 首先测试边有多少
select "count"(*) from epinions   --使用时将表名改变为其它的即可

-- 接着取出所有不同的点，并插入到临时表node_temp中
insert into node_temp(col1) select DISTINCT col1  from epinions  -- 将epinions表中的第一列插入到指定的临时表node_temp中
insert into node_temp(col1) select DISTINCT col2 from epinions   -- 将epinions表中的第二列插入到指定的临时表node_temp中
...
INSERT INTO edge (    
	...
	TRUE
FROM
	epinions   -- 从epinions表中得到总边表，并将边数据插入到指定的edge表中
